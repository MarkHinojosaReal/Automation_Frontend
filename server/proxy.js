...
app.post('/api/tools/terminate-transactions', async (req, res) => {
  try {
    ...
    const METABASE_API_KEY = process.env.METABASE_API_KEY || 'mb_OA03ReuCiuld1BeLyMbuZo/QV60U7YBchhtGxj8xemk=';
    const ARRAKIS_API_KEY = process.env.ARRAKIS_API_KEY;
    const METABASE_CARD_ID = 5342;
    
    if (!ARRAKIS_API_KEY) {
      return res.status(500).json({ error: 'ARRAKIS_API_KEY not configured in .env file' });
    }
